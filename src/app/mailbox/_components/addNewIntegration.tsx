...
export const AddNewIntegration = () => {
  ...
  return (
    <div className="text-left flex flex-col gap-2 max-sm:mx-10 p-4">
      <div>
        ...
        <div className="space-x-2">
          {integrations.map((integration) => (
            <Button
              key={integration.id}
              variant={"outline"}
              className={`rounded-xl max-sm:p-1 max-sm:px-2 max-sm:text-xs max-sm:h-fit max-sm:rounded-lg ${
      ...
      <div className="mt-2 flex flex-col gap-2 items-start w-full">
        ...
        <Input
          ...
          onKeyDown={(e) =>
            e.key === "Enter" && email.length && integrateHandler()
          }
        />
      </div>
      <div className="w-full flex flex-col items-center mt-2">
        <Button
          ...
          size={"sm"}
          disabled={loading || !email || !selectedIntegration}
        >
          {loading && <Loader className="h-4 w-4 animate-spin" />} Continue to
          Integrate
        </Button>
        <p className="mt-4 w-[80%] max-sm:w-full text-center text-sm text-neutral-500">
